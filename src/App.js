...
function App() {
    ...
    useEffect(() => setRepositories(repositories => repositories.map(r => ({data: r.commits, name: r.name}))), []);
        ...
        <div>
            ...
            {activePanel === panels[1].name && (
                <div>
                    ...
                    <EventDrops events={repositories} style={{width: window.outerWidth, height: window.outerHeight}}/>
                </div>
            )}
        </div>
    );
